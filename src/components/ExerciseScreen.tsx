...
type Props = { workout: Workout; exercise: Exercise; set: number; onNext: () => void; onStopWorkout: () => void };
const ExerciseScreen: React.FC<Props> = ({ workout, exercise, set, onNext, onStopWorkout }: Props) => {
	useEffect(() => {
		...
		<>
			...
			<div className="flex gap-1 text-2xl">
				...
				<button onClick={onNext} className="btn flex w-full items-center justify-center gap-1.5 px-24 py-6">
					...
					{set == exercise.sets ? <span>exercise</span> : <span>set</span>}
